...
import com.tvganesh.PlatformJumper.R;
 ...
 * result Intent.
 */
public class BluetoothDeviceDetection extends Activity {
	// ===========================================================
	// Constants
	// ===========================================================
	...
	private final OnItemClickListener mDeviceClickListener = new OnItemClickListener() {
		@Override
		public void onItemClick(final AdapterView<?> pAdapterView,
				final View pView, final int pPosition, final long pID) {
			// Cancel discovery because it's costly and we're about to connect
			BluetoothDeviceDetection.this.mBluetoothAdapter.cancelDiscovery();
			...
			BluetoothDeviceDetection.this.setResult(Activity.RESULT_OK, intent);
			BluetoothDeviceDetection.this.finish();
		}
	};
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context pContext, final Intent pIntent) {
			...
			if (action.equals(BluetoothDevice.ACTION_FOUND)) {
				final BluetoothDevice device = pIntent
						.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
				if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
					BluetoothDeviceDetection.this.mNewDevicesArrayAdapter
							.add(device.getName() + "\n" + device.getAddress());
				}
			} else if (action
					.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED)) {
				BluetoothDeviceDetection.this
						.setProgressBarIndeterminateVisibility(false);
				BluetoothDeviceDetection.this
						.setTitle("Select a device to connect...");
				if (BluetoothDeviceDetection.this.mNewDevicesArrayAdapter
						.getCount() == 0) {
					BluetoothDeviceDetection.this.mNewDevicesArrayAdapter
	...
	protected void onCreate(final Bundle pSavedInstanceState) {
		...
		scanButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(final View v) {
				BluetoothDeviceDetection.this.discoverBluetoothDevices();
		...
		final ListView pairedListView = (ListView) this
		...
		final ListView newDevicesListView = (ListView) this
		...
		final Set<BluetoothDevice> pairedDevices = this.mBluetoothAdapter
				.getBondedDevices();
		if (pairedDevices.size() > 0) {
			this.findViewById(R.id.title_paired_devices).setVisibility(
					View.VISIBLE);
