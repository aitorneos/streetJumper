...
public class NomoEnemy extends Sprite
{
	...
	public NomoEnemy(float pX, float pY, VertexBufferObjectManager vbo, Camera camera, PhysicsWorld physicsWorld)
	{
		super(pX, pY, ResourcesManager.getInstance().enemy2 , vbo);
	...
	{		
		...
		physicsWorld.registerPhysicsConnector(new PhysicsConnector(this, body, true, false)
		{
	        ...
	        public void onUpdate(float pSecondsElapsed)
	        {
				...
				if (getX() >= 1470 && body.getLinearVelocity().x > 0)
				{
					body.setLinearVelocity(-1.0f, 0.0f);
				}
				
				if (getX() <= 1425 && body.getLinearVelocity().x < 0)
				{
					body.setLinearVelocity(1.0f, 0.0f);
				}
